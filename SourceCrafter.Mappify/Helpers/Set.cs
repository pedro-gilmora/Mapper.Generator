...
public abstract class Set<TValue> : IEnumerable<TValue>
{
    public static Set<TValue> Create<TKey>(
        ...
        IEqualityComparer<TKey>? comparer = null) => new Set<TKey, TValue>(keySelector, comparer);
...
public class Set<TKey, TValue>(Func<TValue, TKey> keySelector, IEqualityComparer<TKey>? comparer = null) : Set<TValue>
{
    private readonly Func<TKey, int> _getHashCode = comparer is null
            ? default(TKey) is int or short or byte or uint or ushort
                ? e => Convert.ToInt32(e)
                : e => Convert.ToInt32(EqualityComparer<TKey>.Default.GetHashCode(e))
            : e => Convert.ToInt32(comparer.GetHashCode(e));

    private readonly Func<TKey, TKey, bool> _equals = comparer is null
    ...
    public ref TValue GetValueOrInserter(TKey key, out Action<TValue> insertor)
    {
        ...
        insertor = item =>
        {
            hashCode = _getHashCode(key = keySelector(item)!);
            var entries = _entries!;
            ref var bucket = ref GetBucket((uint)hashCode);
            int index;
            if (_freeCount > 0)
    ...
    public bool TryGetValue(TKey key, out TValue val)
    {
        ...
        do
        {
            ...
            if (entry.id == hashCode && _equals(entry.Key, key))
            {
                val = entry.Value;
                return true;
            }
